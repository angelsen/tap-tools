...
def send_command(
    ...
    with process_scan(pane.pane_id):
        ...
        if modified is None:
            return _build_result(pane, command, "cancelled", start_time, 
                               error="Command cancelled by handler")
    ...
    try:
        ...
        if not success:
            return _build_result(pane, command, "failed", start_time,
                               error="Failed to send command to pane")
        ...
        while time.time() - start_time < actual_timeout:
            with process_scan(pane.pane_id):
                ...
                if not current_handler.during_command(pane, elapsed):
                    return _build_result(pane, command, "aborted", start_time,
...
def send_keys(pane: Pane, *keys: str, enter: bool = False) -> dict[str, Any]:
    ...
    if not success:
        with process_scan(pane.pane_id):
            return _build_result(pane, keys_str, "failed", start_time, 
