"""tmux streaming operations - core of termtap."""
import time
...
class StreamHandle:
    ...
    def start(self) -> bool:
        ...
        code, _, _ = run_tmux([
            "pipe-pane", "-o", "-t", self.pane_id, shell_cmd
        ])
        return code == 0
...
def get_pane_for_session(session: str) -> str:
    ...
    # Return in session:window.pane format
    # This format works reliably with -t flag
    return f"{session}:0.0"


def send_command(pane_id: str, command: str) -> str:
