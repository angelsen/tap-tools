...
PUBLIC API:
  (None - all functions are internal)
"""
...
from .tree import _get_process_info, _get_child_processes
...
def _check_stdin_wait(pid: int) -> bool:
    ...
    try:
        ...
        result = subprocess.run(["lsof", "-p", str(pid), "-a", "-d", "0"], capture_output=True, text=True)
...
def _get_wait_channel(pid: int) -> Optional[str]:
...
def _is_ready_for_input(pid: int) -> bool:
    ...
    return _check_stdin_wait(pid)


def _get_process_state(pid: int) -> Optional[_ProcessState]:
    ...
    # Check stdin
    waiting_on_stdin = _check_stdin_wait(pid)
    ...
    open_files = []
    try:
        result = subprocess.run(["lsof", "-p", str(pid), "-F", "n"], capture_output=True, text=True)
        if result.returncode == 0:
            # Parse lsof output (each line starts with 'n')
            open_files = [line[1:] for line in result.stdout.splitlines() if line.startswith("n")]
    except (subprocess.CalledProcessError, FileNotFoundError):
        pass
...
def _wait_for_ready_state(pid: int, timeout: float = 30.0, poll_interval: float = 0.1) -> bool:
...
def _is_repl_ready(pid: int) -> bool:
    ...
    if info.name not in repl_names:
        return _is_ready_for_input(pid)  # Fallback to general check
    ...
    return _is_ready_for_input(pid)
