...
TESTING LOG:
Date: 2025-08-07, Updated: 2025-10-05
System: Linux 6.12.39-1-lts, 6.12.43-1-lts, 6.15.6-arch1-1
...
Tracking: ~/.termtap/tracking/20250807_212502_printTesting_system_Python
         ...
         ~/.termtap/tracking/20251005_031017_print__hello_m8 (Kernel 6.15.6-arch1-1)

Observed wait_channels:
- do_select: Python REPL waiting for input (ready)
- do_sys_poll: Python REPL polling for input (ready)
- poll_schedule_timeout.constprop.0: Python polling (ready, newer kernels 6.15+)
...
Notes:
- do_sys_poll is the most common ready state for Python 3.12.11 on kernel 6.12.x
- poll_schedule_timeout.constprop.0 replaces do_sys_poll on kernel 6.15+
- .constprop.X suffix is compiler optimization (constant propagation)
...
class _PythonHandler(ProcessHandler):
    ...
    def is_ready(self, pane: Pane) -> tuple[bool | None, str]:
        ...
        # Newer kernels (6.15+) use poll_schedule_timeout with compiler suffix
        if wc and wc.startswith("poll_schedule_timeout"):
            return True, "Python polling for input"
