...
from .core import execute, get_result, ExecutorState
...
@app.command(
    ...
    fastmcp={
        "type": "tool",
        "description": "Execute command in tmux session"
    }
)
def bash(
    state: TermTapState,
    ...
    # Determine hover preference
    if hover is None:
        hover = (
            check_tmux_hover_env() or
            should_hover(command, config)
        )
    ...
    # Create hover callback if needed
    hover_callback = None
    if config.get("hover_patterns"):
        ...
        def callback(pattern, output):
            ...
            # Use target as session name (execute() will determine actual session)
            return pattern_hover_callback(
                pattern, output, 
                target, 
                command
            )
...
@app.command(
    display="table",
    headers=["Session", "Created", "Attached"],
...
def ls(state: TermTapState) -> List[dict]:
    ...
    for s in state.sessions:
        sessions.append({
            "Session": s.name,
            "Created": s.created,
            "Attached": "Yes" if s.attached != "0" else "No"
        })
    return sessions
...
@app.command(
    ...
    fastmcp={
        "type": "tool",
        "description": "Get status of running command with process info"
    }
)
def status(state: TermTapState, cmd_id: str) -> dict:
    ...
    # Get process info if command is running
    process_info = None
...
def abort(state: TermTapState, cmd_id: str) -> str:
    ...
    from .core.execute import abort_command
...
@app.command(
    display="table",
    headers=["ID", "Command", "Session", "Elapsed"],
    fastmcp={"enabled": False}
)
...
def dashboard_display(data, meta):
    ...
    if "sessions" in data and data["sessions"]:
        components.append(box(
            table(data["sessions"], headers=["Session", "Attached"]),
    ...
    if "commands" in data and data["commands"]:
        components.append(box(
            table(data["commands"], headers=["ID", "Command", "Elapsed"]),
