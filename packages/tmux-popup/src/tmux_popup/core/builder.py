...
class ShellBuilder:
    ...
    def add_command(self, cmd: List[str], capture: bool = False, result_name: Optional[str] = None) -> str:
        ...
        cmd_str = " ".join(
            arg if arg.startswith("$") else shlex.quote(arg) 
            for arg in cmd
        )
    ...
    def add_style(self, content_var: str, width: Optional[str] = None, height: Optional[str] = None,
                  border: str = "none", align: Optional[str] = None, padding: Optional[str] = None,
                  margin: Optional[str] = None, result_name: Optional[str] = None) -> str:
